...
const FlashSaleManagement = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setShowForm(false);
            setFormData({ productId: '', discountPercentage: 30, startTime: '', endTime: '', maxQuantity: 50 });
            fetchFlashSales();
        } catch (error) {
    ...
    return (
        <div>
            ...
            <div style={{ display: 'grid', gap: '1rem' }}>
                {flashSales.map(sale => (
                    <div key={sale._id} style={{ border: '1px solid #ddd', borderRadius: '8px', padding: '1rem' }}>
                        <div><strong>{sale.product.name}</strong> - {sale.discountPercentage}% OFF</div>
                        <div>Ends: {new Date(sale.endTime).toLocaleString()}</div>
                        <div>Remaining: {sale.remainingQuantity} left</div>
                        <button 
                ...
                {flashSales.length === 0 && (
                    <div style={{ textAlign: 'center', padding: '2rem', color: '#999' }}>
                        No active flash sales. Create one to attract customers!
                    </div>
                )}
            </div>
        </div>
